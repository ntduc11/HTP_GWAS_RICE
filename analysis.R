...
control.plant.growth.modeling <- function(input, plant.id=NULL, genotype.id=NULL, treatment='control', min.data.points=10,
	...
	if(!is.nothing(plant.id)){
		...
		stop("You should specify a plant.id or a genotype.id\n")
	}
	## checking data
	...
	data.y[data.y==0] <- NA
	if(is.null(modeling.data) | nrow(modeling.data) < 1 | sum(!is.na(data.y)) < min.data.points){
	...
	pred.time <- sort(unique(modeling.data[, col.day]))
	...
	if(plot){
		xrange <- range(0, pred.time)
		yrange <- range(data.y, mlogistic$kmax*1.1, na.rm=TRUE) * absmax				
		...
		abline(v=mlogistic$inflection, h=mlogistic$kmax*absmax, lty=mlogistic$lty, col=mlogistic$color)
		kmax.exp <- inflection.exp <- vector('expression',1)
		inflection.exp[1] <- substitute(expression(italic(T)[inflection] == II), 
						list(II=format(mlogistic$inflection, digits=4)))[2]
		kmax.exp[1] <- substitute(expression(italic(K)[max] == KK), 
						list(KK=format(mlogistic$kmax*absmax, digits=4, scientific=TRUE)))[2]
		text(mlogistic$inflection, yrange[1], labels=inflection.exp, pos=4)
		text(xrange[2]*0.8, mlogistic$kmax*absmax, labels=kmax.exp, pos=3)
		
		ltys <- c(mlinear$lty, mexponential$lty, mmonomolecular$lty, mgompetz$lty, mlogistic$lty)
		cols <- c(mlinear$color, mexponential$color, mmonomolecular$color, mgompetz$color, mlogistic$color)
		lgds <- vector('expression',5)
		lgds[1] <- mlinear$legend
		lgds[2] <- mexponential$legend
		lgds[3] <- mmonomolecular$legend
		lgds[4] <- mgompetz$legend
		lgds[5] <- mlogistic$legend
		my.legend("topleft", legend=lgds, cex=0.8, col=cols, text.col=cols, lty=ltys, title="Models")
	}
	
	invisible(
		list(Plant=paste(iplants, collapse="|"), Genotype=paste(genotypes, sep="|"), 
			Inflection=mlogistic$inflection, Kmax=mlogistic$kmax*absmax, base=absmax,
			IntrinsicGrowth=mlogistic$intrinsic.growth, 
			InflectionGR=mlogistic$inflection.growth.rate*absmax, 
			InflectionRGR=mlogistic$inflection.relative.growth.rate, 
			InflectionGrowth=mlogistic$inflection.growth*absmax, 
			Prediction=cbind(Time=pred.time, Pred=mlogistic$prediction*absmax),
			linear=mlinear, exponential=mexponential, monomolecular=mmonomolecular,
			gompetz=mgompetz, logistic=mlogistic)
	)
}
...
stressed.plant.growth.modeling <- function(input, plant.id=NULL, genotype.id=NULL, treatment='stress', 
									first.stress.day=NA, last.stress.day=NA, inluded.days.before.stress=5, min.data.points=10,
									proxy.trait="volume.fluo.iap", col.plant="PlantID", col.treatment="Treatment", 
									col.genotype="Genotype", col.day="DAS", col.time="Realtime",
									xlab=col.day, ylab=proxy.trait, plot=TRUE, echo=TRUE){
	column.check(input, col.plant, col.genotype, col.day, col.time, proxy.trait)
	...
	phase1 <- which(pred.time >= first.stress.day-inluded.days.before.stress & pred.time <= last.stress.day)
	phase2 <- which(pred.time > last.stress.day)
	data.time1 <- data.time[phase1]
	data.time2 <- data.time[phase2]
	pred.time1 <- pred.time[phase1]
	pred.time2 <- pred.time[phase2]
	...
	mlinear <- linear.model(data.y2, data.time2, pred.time2, lty=6, col="#E4007F")
	...
	if(plot){
		...
		IPs <- c(mbellshape3$inflection.before, mbellshape3$inflection.after, mbellshape3$t.max)
		...
		abline(v=IPs, lty=mbellshape3$lty, col=mbellshape3$color)
		####
		...
		rect(Tmax.obs, 0, mbellshape3$t.max, mbellshape3$t.max.pred, col=col.alpha, border=mbellshape3$color)
		####
		abline(v=c(first.stress.day, last.stress.day), lty=5, col='grey')
	...
	invisible(
		list(Plant=paste(iplants, collapse="|"), Genotype=paste(genotypes, collapse="|"), 
			Bell.IP.before=mbellshape3$inflection.before, Bell.IP.after=mbellshape3$inflection.after, 
			Bell.IP.before.pred=mbellshape3$inflection.before.pred, 
			Bell.IP.after.pred=mbellshape3$inflection.after.pred, 
			Bell.t.max=mbellshape3$t.max, Bell.t.max.pred=mbellshape3$t.max.pred, 
			Stress.Tmax.obs=Tmax.obs, Tmax.diff=Tmax.obs-mbellshape3$t.max,
			Linear.growth.rate=mlinear$growth.rate, Linear.initial.RGR=mlinear$initial.relative.growth.rate, 
			Prediction=cbind(Time=c(pred.time1, pred.time2), Pred=c(mbellshape3$prediction[phase1], mlinear$prediction)),
